...
#Список внешних сил, действующих на груз
forces = [
...
def plot_results(t_eval, x_analytical_under, x_num_under, x_analytical_over, x_num_over, force_name):
    ...
    plt.subplot(1, 2, 1)
    plt.plot(t_eval, x_analytical_under(t_eval), label='Аналитическое')
    plt.plot(t_eval, x_num_under, '--', label='Численное')
    ...
    plt.subplot(1, 2, 2)
    plt.plot(t_eval, x_analytical_over(t_eval), label='Аналитическое')
    plt.plot(t_eval, x_num_over, '--', label='Численное')
    plt.title(f'h^2 > 4km Case\n{force_name}')
    plt.xlabel('Time (s)')
    plt.ylabel('Displacement (m)')
    plt.legend()
